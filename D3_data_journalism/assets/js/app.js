...
// data binding and charts
  ...
  var textGroup = chartGroup.selectAll("#stateLabel")
    .data(smokingData)
    .enter()
    .append("text")
    .attr("id", "stateLabel")
...
function setXScale(data, axisChoice){
    ...
    .domain([d3.min(data, d => d[axisChoice])*0.8 , d3.max(data, d => d[axisChoice])*1.2])
    .range([0, width]);
    
    console.log(`x min ${d3.min(data, d => d[axisChoice])}`);
    console.log(`x max ${d3.max(data, d => d[axisChoice])}`);
...
function setYScale(data, axisChoice){
    ...
    console.log(`y min ${d3.min(data, d => d[axisChoice])}`);
    console.log(`y max ${d3.max(data, d => d[axisChoice])}`);
